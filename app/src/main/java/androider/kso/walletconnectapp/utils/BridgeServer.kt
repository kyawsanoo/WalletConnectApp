...
class BridgeServer(moshi: Moshi) : WebSocketServer(InetSocketAddress(PORT)) {
    ...
    override fun onOpen(conn: WebSocket?, handshake: ClientHandshake?) {
        Log.d("BridgeServer", "onOpen: ${conn?.remoteSocketAddress?.address?.hostAddress}")
    }

    override fun onClose(conn: WebSocket?, code: Int, reason: String?, remote: Boolean) {
        Log.d("BridgeServer", "onClose: ${conn?.remoteSocketAddress?.address?.hostAddress}")
        conn?.let { cleanUpSocket(it) }
    }

    override fun onMessage(conn: WebSocket?, message: String?) {
        Log.d("BridgeServer", "Message: $message")
        try {
            ...
            message?.also {
                ...
                when (type) {
                    "pub" -> {
                        ...
                        if (!sendMessage) {
                            Log.d("BridgeServer", "Cache message: $message")
                    ...
                    "sub" -> {
                        ...
                        pubsCache[topic]?.let { cached ->
                            Log.d("BridgeServer", "Send cached: $cached")
                            conn.send(cached)
                        }
                    }
    ...
    override fun onStart() {
        Log.d("BridgeServer", "Server started")
        connectionLostTimeout = 0
    }

    override fun onError(conn: WebSocket?, ex: Exception?) {
        Log.d("BridgeServer", "onError")
        ex?.printStackTrace()
        conn?.let { cleanUpSocket(it) }
    }
